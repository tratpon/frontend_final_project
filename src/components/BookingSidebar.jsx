...
export default function BookingSidebar({ serviceID }) {
  ...
  const { data: monthSlots = [] } = useQuery({
    queryKey: ["monthSlots", serviceID, year, month],
    queryFn: () => fetchSlotDaysInMonth(serviceID, year, month + 1),
    enabled: !!serviceID,
  });

  /* 📌 convert เป็น set ของวัน */
  const daysWithSlots = new Set(
    monthSlots.map((slot) => slot.day)
  );
